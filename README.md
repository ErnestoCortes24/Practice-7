...
## Conclusion
In this practice, I learned about the functionality of a keyboard. With the Curiosity board is not complex to programming this task, so I would like to take an important points: The keys works with inverse logic (from one to cero) to the controller knows what keys are being pressing. To cycle through the bits, we need a LAT# to reed and averwrite the last value, an or with an hexadecimal number to show the shift of each script. We need a switch loop to reed the value of the port to know the pressing key thank the row and the column, with an hexadécimal number that we declared in the left digit, we declare the row, and the right digit the columns,
